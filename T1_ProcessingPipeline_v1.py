...
from pydra.engine.specs import SpecInfo, BaseSpec, ShellSpec, ShellOutSpec, File, Directory
from pydra.tasks.mrtrix3.v3_0 import labelconvert, labelsgmfix, fivett2vis, fivettgen_hsvs, mrconvert
...
from fileformats.generic import Directory as GenericDirectory
...
wf = Workflow(name='T1_ProcessingPipeline', input_spec=input_spec, cache_dir=output_path, output_spec=output_spec) 
...
##############################################
# JOIN TASK to identify and set parcellation #
##############################################
...
wf.add(join_task_catalogue(FS_dir=wf.FastSurfer_task.lzout.subjects_dir_output, parcellation=wf.lzin.parcellation, freesurfer_home=freesurfer_home, name="join_task"))
...
########################
# Set the workflow outputs as the result of the join_task
...
wf.set_output(("parcellation_image", wf.SGMfix_task.lzout.output.cast(ImageFormat)))
...
result = wf(
    ...
    parcellation="desikan", #   destrieux hcpmmp1 yeo17fs yeo7fs 
    plugin="serial",
)
